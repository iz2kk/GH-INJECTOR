...
{ 
	0x55, 0x8B, 0xEC, 0x56, 0x8B, 0x75, 0x08, 0x85, 0xF6, 0x74, 0x29, 0x8B, 0x46, 0x04, 0x85, 0xC0, 0x74, 0x22, 0x8D, 0x4E, 0x14, 0x56, 0x89, 0x4E, 0x10, 0x8D, 0x4E, 0x0C, 0x51, 0x6A, 0x00, 0x6A, 0x00, 0xFF, 
	0xD0, 0x89, 0x46, 0x08, 0x8B, 0x06, 0xC7, 0x46, 0x04, 0x00, 0x00, 0x00, 0x00, 0x5E, 0x5D, 0xC2, 0x04, 0x00, 0x33, 0xC0, 0x5E, 0x5D, 0xC2, 0x04, 0x00
};
...
DWORD _LdrLoadDll_WOW64(const wchar_t * szDllFile, HANDLE hTargetProc, LAUNCH_METHOD Method, DWORD Flags, HINSTANCE & hOut, DWORD & LastError)
{
	...
	BYTE * pArg = ReCa<BYTE*>(VirtualAllocEx(hTargetProc, nullptr, sizeof(LDR_LOAD_DLL_DATA_WOW64) + sizeof(LdrLoadDll_Shell_WOW64) + 0x10, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
	if (!pArg)
	{
		LastError = GetLastError();
